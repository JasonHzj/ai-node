...
async function runInitialSyncForUser(io, userId, account, startDate) {
    ...
    try {
        ...
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('数据库检查超时。')), 15000));
        ...
        io.to(userId.toString()).emit('sync_progress', {
            progress: 10,
            message: `[${account.account_name}] 广告同步完成!`
        });
        await delay(500);
        ...
        while (currentStartDate <= GLOBAL_END_DATE) {
            ...
            io.to(userId.toString()).emit('sync_progress', {
                ...
                message: `[${account.account_name}] 同步 ${formattedStart} -> ${formattedEnd} 数据...`
            });
        ...
        io.to(userId.toString()).emit('sync_complete', {
            message: `账户 [${account.account_name}] 的历史数据已同步完成！`
        });
